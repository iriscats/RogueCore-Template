...
APatrolBot::APatrolBot(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer) {
    ...
    this->LaserBeamNS = CreateDefaultSubobject<UNiagaraComponent>(TEXT("LaserBeamIndicatorNS"));
    this->TearingGroundParticlesNS = CreateDefaultSubobject<UNiagaraComponent>(TEXT("TearingGroundParticlesNS"));
    ...
    this->LaserBeamNS->SetupAttachment(mesh);
    this->RollingAudio->SetupAttachment(mesh);
    this->TearingGroundParticles->SetupAttachment(RootComponent);
    this->TearingGroundParticlesNS->SetupAttachment(RootComponent);
}
